...
void KPD_init(void)
{
	...
	/*Set columns & Rows Direction*/
	DIO_SETpinDir(KPD_ColPORT,KPD_COLUMN0_PIN,DIO_OUTPUT);
	DIO_SETpinDir(KPD_ColPORT,KPD_COLUMN1_PIN,DIO_OUTPUT);
	DIO_SETpinDir(KPD_ColPORT,KPD_COLUMN2_PIN,DIO_OUTPUT);
	DIO_SETpinDir(KPD_RowPORT,KPD_ROW0_PIN,DIO_INPUT);
	DIO_SETpinDir(KPD_RowPORT,KPD_ROW1_PIN,DIO_INPUT);
	DIO_SETpinDir(KPD_RowPORT,KPD_ROW2_PIN,DIO_INPUT);
	DIO_SETpinDir(KPD_RowPORT,KPD_ROW3_PIN,DIO_INPUT);

	/*Set Rows to work in pull up mode*/
	DIO_SETpinVal(KPD_RowPORT,KPD_ROW0_PIN,DIO_HIGH);
	DIO_SETpinVal(KPD_RowPORT,KPD_ROW1_PIN,DIO_HIGH);
	DIO_SETpinVal(KPD_RowPORT,KPD_ROW2_PIN,DIO_HIGH);
	DIO_SETpinVal(KPD_RowPORT,KPD_ROW3_PIN,DIO_HIGH);

	/*Deactivate columns by setting them high*/
	DIO_SETpinVal(KPD_ColPORT,KPD_COLUMN0_PIN,DIO_HIGH);
	DIO_SETpinVal(KPD_ColPORT,KPD_COLUMN1_PIN,DIO_HIGH);
	DIO_SETpinVal(KPD_ColPORT,KPD_COLUMN2_PIN,DIO_HIGH);
#endif
}
...
uint8_t KPD_u8GetPressedKey(void)
{
	...
	{
		/*Activate Columns*/
		DIO_SETpinVal(KPD_ColPORT,KPD_COLUMNS[KPD_COLUMNS_Index],DIO_LOW);
		/*Iterate to check row's values*/
		for(KPD_ROWS_Index=0;KPD_ROWS_Index<Row_num4_3;KPD_ROWS_Index++)
		{
			/*get the row state*/
			DIO_GETpinVal(KPD_RowPORT,KPD_ROWS[KPD_ROWS_Index],&ROW_PIN_State);
			/*if button pressed*/
			if(DIO_LOW==ROW_PIN_State)
			{
				/*put the button number in key value*/
				...
				/*deactivate the column*/
				DIO_SETpinVal(KPD_ColPORT,KPD_COLUMNS[KPD_COLUMNS_Index],DIO_HIGH);
				return local_u8Pressed;
			}
		}
		/*Deactivate current column*/
